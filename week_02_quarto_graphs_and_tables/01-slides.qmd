...
# Quarto, websites, and PRs

## This week

- Quarto: one plain-text file, rendered to PDF, slides, or a website.
- Graphs with `ggplot2` and tables with `tinytable`, from a Parquet file that Python made.
- A website on GitHub Pages.
- Pull requests, reviews, and conflicts.

::: {.notes}
- Two hours of doing, 30 min of Annie; 30 min Q&A.
- The demonstration file is `02-demo.qmd`; the website is `website_example/`.
:::
...
Prose in Markdown, with a citation [@healy2019]
and a cross-reference to @fig-users.

```{r}
#| label: fig-users
#| fig-cap: "People in shelters each night"
ggplot(daily, aes(date, service_users)) + geom_line()
```
````

::: {.notes}
- Header says what to make. Prose is Markdown. Chunks run at render time.
- Change `pdf` to `html` or `revealjs` and nothing else changes.
:::

## Why this matters for the Donaldson paper

- Everything (tables, graphs) in the PDF comes from code that ran on the data 😎
- When we change the data we can render it again (or have a GitHub ACtions to do that automatically), and then everything updates 😎
- A reader with the repo can check any figure 😎
- Pasted-in figures cannot be checked and go stale 😢

## You can use both Python and R

```
aggregate_shelter.py  --polars-->  data/shelter_daily.parquet  --arrow-->  02-demo.qmd (R)
```

- For instance, it's common to use Python for scraping or interacting with an API: download, clean, aggregate, write Parquet.
- Then you could use R to make graphs and tables: `arrow::read_parquet()`, `ggplot2`, `tinytable`.
- Parquet keeps the types, so a date is still a date!
...
- Data, then aesthetics (`aes`), then a geometry, then labels
- We use a "grammar" of graphics.

## `tinytable` hello world!

```r
latest |>
  tt(digits = 3) |>
  format_tt(j = "Occupancy", fn = scales::label_percent(0.1)) |>
  format_tt(escape = TRUE)
```

- Caption and cross-reference come from the chunk options `label: tbl-...` and `tbl-cap`.

## A website is just a type of doc
...
- `quarto render` builds every page into `docs/`.
- GitHub, Settings, Pages, deploy from `main`, folder `/docs`.
- `output-dir: docs` makes it easier to deploy from GitHub Pages.

## Pull requests

```bash
git checkout -b week_03_group_1
git add . && git commit -m "Add group 1 slides"
git push -u origin week_03_group_1
```

- Then "Compare & pull request" on GitHub
- Someone else reads the diff and comments before it merges
...
- Two people changed the same lines; Git asks you to choose
- Keep what you want, delete the three marker lines, `git add`, `git commit`
- Prevention: `git pull` before you start, and small PRs
...
![Figure 1.1 from @cleveland1994 [p. 4].](img/sunspots_1.png)
...
![Figure 1.1 from @cleveland1994 [p. 4].](img/sunspots_2.png)
...
![Mean and standard deviation for Anscombe's quartet.](img/anscombe_1.png){width=60%}
...
![ Recreation of Anscombe's Quartet.](img/anscombe_2.png)
...
![Distribution of age-groups in the 1997-2001 British Election Panel Study.](img/bar_chart_1.png)
...
![Distribution of age-group by gender, and vote preference, in the 1997-2001 British Election Panel Study.](img/bar_chart_2.png)
...
![Relationship between inflation and GDP growth for Australia, Ethiopia, India, and the United States.](img/scatterplot.png)
...
![United States GDP growth (1961-2020)](img/lineplot.png)
...
![Distribution of GDP growth in Ethiopia (1960-2020)](img/histogram_1.png)
...
![Distribution of GDP growth in Ethiopia (1960-2020)](img/histogram_2.png)
...
- This is one of the reasons that @DenbyMallows2009 consider histograms to be especially valuable as exploratory tools.
:::
...
![Data drawn from beta distributions with different parameters](img/boxplot.png)
...
- One reason for using graphs is that they help us understand and embrace how complex our data are, rather than trying to hide and smooth it away [@Armstrong2022]. One appropriate use case for boxplots is to compare the summary statistics of many variables at once, such as in @Bethlehem2022. But boxplots alone are rarely the best choice because they hide the distribution of data, rather than show it. The same boxplot can apply to very different distributions. 
...
![A chart with a considerable amount of junk in it.](img/bad_taste.png)
...
![A crisis of faith in democracy? (New York Times.)](img/bad_data_1.png)
...
![Perhaps the crisis has been overblown. (Erik Voeten.)](img/bad_data_2.png)
...
![A 3-D Column Chart created in Microsoft Excel for Mac. Although it may seem hard to believe, the values shown in the bars are 1, 2, 3, and 4.)](img/bad_perception.png)
